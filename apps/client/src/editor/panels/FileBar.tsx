...
/**
 * 文件栏 = 场景级操作:新建空白(清空物体 / 分镜 / 底图 + 复位视角) / 保存 / 读取 / 设置。
 * 物体级操作(移动/旋转/复制/拆分/删除)在选中物体后的悬浮工具条(SelectionHud)里。
 */
export function FileBar({ compact }: { compact?: boolean }) {
  ...
  return (
    <View style={styles.row}>
      ...
      {/* 保存 = 把当前构图(含分镜/底图)存成 .json 文件;读取 = 从文件还原 */}
      <Button
        label="💾"
        compact={compact}
        disabled={!hasObjects}
