...
| D16 | 多机位                       | 多机位**数据层 day 1 就支持**(`shots[]`);"一键批量出图"只是个按钮,非独立阶段        | 见 §6.1                                        |
...
interface Scene {
  ...
  actors: Actor[];
  props: Prop[];
  shots: Shot[]; // 一个场景存多个分镜/机位(杀手级功能;原 cameras)
  activeShotId: string;
  createdAt: number;
  updatedAt: number;
}
...
interface Shot {
  // 原 Camera;重构后改名,并去掉 preset、加入 aspect/thumbnail
  id: string;
  name: string; // "正面"、"侧面"
  position: [number, number, number];
  target: [number, number, number]; // look-at 点
  fov: number;
  aspect: string; // 画幅比例 id(如 '16:9'),还原构图用
  thumbnail: string | null; // 取景缩略图(PNG dataURL);分镜条展示
}
```
...
- **"看向目标"模式**:相机始终框住选中角色。
- **一个场景存多个机位**:数据层 day 1 就支持(`shots[]`),"加机位"零成本。在此之上,"一键对所有机位批量出图"只是一个按钮(循环渲染 N 张截图 → 提交 N 个生成任务),**不是独立开发阶段(D16)**。同一布景多角度出图,对讲故事/漫画是杀手级功能。
...
| **1. 最小编辑器 + Firebase** | 进行中    | Three.js 载入资产,摆放/旋转/贴地,相机 orbit + **分镜截存/回放**,一键截图 → 后端 → **出图(已端到端接通,BYOK Gemini)**。Firebase Auth/Firestore/Storage **客户端仍待接**(部署件 + 后端 Admin 鉴权已就绪)。**只做 Web** | Web 单端跑通闭环最快,验证产品成立 |
...
- **分镜(shot)**:把当前取景(机位 + 画幅)截存成的一格,带缩略图,可回放/切换。(早期的「相机电影预设」computePreset 已在重构中移除。)
