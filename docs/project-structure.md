...
│       └── src/
│           ├── App.tsx
│           ├── editor/            # 3D 编辑器、相机、导入/导出、渲染分发
│           │   ├── panels/        # 资产库、相机、预览、变换工具
│           │   ├── camera.ts      # 镜头预设(实时算位)
│           │   ├── sceneFile.ts   # 本地场景 JSON 保存/读取
│           │   ├── importModel.ts # glTF/GLB 运行时导入
│           │   └── splitModel.ts  # 导入模型按部件拆分
│           ├── store/             # zustand:对象/选择/相机/用户模型
│           └── ui/                # 通用 UI 与设计 token
│
├── packages/
...
│   │       └── index.ts
│   └── resource-library/          # 文件化 3D 资源库(贡献入口)
│       ├── models/<id>/meta.json  # 一个模型 = 一个文件夹
│       ├── models/<id>/model.glb  # 真实模型可选,需内嵌纹理
│       ├── src/generated/catalog.ts # `pnpm gen` 自动生成
│       └── CREDITS.md             # pnpm gen 自动生成许可署名
│
└── services/
...
- `packages/resource-library`:资源库保持纯数据,不依赖 React / three;前端按 `source.kind` 渲染。
- `pnpm gen`:扫描 `models/<id>/meta.json`,生成 catalog、拷贝 dev 模型、更新 `CREDITS.md`。
